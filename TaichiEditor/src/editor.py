...
from PyQt5.QtWidgets import QMainWindow, QFileDialog, QMessageBox, QLabel, QToolBar
from .menu.menu_bar import MenuBar
from .menu.tool_bar import ToolBar
...
class Editor(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Taichi Editor")
    ...
    def __add_toolbar(self):
        toolbar_bookmarks = Bookmark(self)
        file_toolbar = QToolBar()
        file_toolbar.setIconSize(QSize(18, 18))
        self.addToolBar(file_toolbar)
        file_toolbar.addAction(toolbar_bookmarks.FILE["new"])
        file_toolbar.addAction(toolbar_bookmarks.FILE["open_file"])
        file_toolbar.addAction(toolbar_bookmarks.FILE["save"])
        file_toolbar.addAction(toolbar_bookmarks.FILE["save_as"])
        file_toolbar.addAction(toolbar_bookmarks.EDIT["undo"])
    ...
    def __connectSignals(self):
        ...
        self._menu_bar.compile.connect(self.compile_taichi)
    ...
    def compile_taichi(self):
        ...
        self.status_bar.showMessage(f"compiling file {FILE_NAME}...")
        ...
        is_compiled = self._compile_taichi(FULL_PATH)

        if not is_compiled:
            self.status_bar.showMessage("Error while compiling "
                                        f"\"{FILE_NAME}\"")
            msg_box = QMessageBox(QMessageBox.Information, 'Taichi Editor', 
            ...
            msg_box = QMessageBox(QMessageBox.Information, 'Taichi Editor', 
                                        f"File \"{FILE_NAME}\" compiled successfully")
            msg_box.exec_()  

    def _compile_taichi(self, path: str):
        try:
            command = "python " + path
            os.system(command)
    ...
    def open_file(self):
        print(os.getcwd())
        file_path = QFileDialog.getOpenFileName(self, "Open file", directory="../")[0]
        if file_path == "":
            return
